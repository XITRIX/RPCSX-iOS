...
std::string logLevelToText(logs::level level) {
    switch (level) {
...
void log(logs::level level, std::string prefix, std::string message) {
    std::string log = fmt::format("[{}] {} {}", prefix, logLevelToText(level), message);
    printf("%s\n", log.c_str());
}
...
struct LogListener : logs::listener {
  ...
  void log(u64 stamp, const logs::message &msg, const std::string &prefix,
    ...
    std::string prio = logLevelToText(static_cast<logs::level>(msg));
    std::string message = fmt::format("[RPCS3] {} {}", prio, text);
    printf("%s\n", message.c_str());
  }
...
[[noreturn]] void report_fatal_error(std::string_view _text,
  ...
  fmt::append(buf, "\nDate: \"%s\"", std::chrono::system_clock::now());
    ...
    log(logs::level::fatal, "RPCS3", buf);
...
static void setupCallbacks() {
  Emu.SetCallbacks({
      ...
      .close_gs_frame = [](auto...) {},
      .get_gs_frame = [] { return nullptr; /*std::make_unique<GraphicsFrame>();*/ },
      .get_camera_handler =
...
//          return std::make_shared<dualsense_pad_handler>();
      ...
      .get_msg_dialog = [] { return nullptr; /*std::make_shared<MessageDialog>();*/ },
